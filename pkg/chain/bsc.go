...
func NewBSCClient(cfg config.Config, hdwallet *hdwallet.Wallet) (Chain, error) {
...
func (ch *BSC) Balance(address string) (float64, error) {
...
func (ch *BSC) Transfer(fromAcc accounts.Account, toAcc accounts.Account, amount float64, token model.Token, nonce int, all bool) (*types.Transaction, float64, error) {
	var (
		t   *types.Transaction
		err error
	)
...
func (ch *BSC) transfer(fromAcc accounts.Account, toAcc accounts.Account, amount float64, prevTxNonce int, all bool) (*types.Transaction, float64, error) {
	balance, err := ch.Balance(fromAcc.Address.Hex())
	if err != nil {
		return nil, 0, err
	}
	fmt.Printf("[BNB | transfer][%s - %s]: amount %v | balance %v | all - %v\n", fromAcc.Address.Hex(), toAcc.Address.Hex(), amount, balance, all)
...
func (ch *BSC) transferToken(fromAcc accounts.Account, toAcc accounts.Account, amount float64, token model.Token, prevTxNonce int, all bool) (*types.Transaction, float64, error) {
	priv, _ := ch.hdwallet.PrivateKeyHex(fromAcc)
	privateKey, err := crypto.HexToECDSA(priv)
	if err != nil {
		return nil, 0, err
...
func (ch *BSC) erc20TokenBalance(address string, token model.Token) (float64, error) {
...
func (ch *BSC) Balances(address string, tokens []model.Token) (map[string]float64, error) {
	balances := make(map[string]float64, 0)
	for _, token := range tokens {
		key := strings.ToUpper(token.Symbol)
		switch key {
		case "BNB":
			bnbBalance, err := ch.Balance(address)
			if err != nil {
				return nil, err
			}
			balances[key] = bnbBalance
		default:
